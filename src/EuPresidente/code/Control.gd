...
var falas = [
	"Prazer, " + Global.nomePessoa +"! Chamamos você aqui porque, na noite passada, "+Global.genero[0]+" atual presidente do Brasil, sumiu. Não sabemos onde "+Global.genero[1]+" está e, desde então, o país está sem um chefe do Poder Executivo.",
	...
	"Eu era " +Global.genero[2]+" do seu irmão e posso garantir que muitos desafios aparecerão pelo caminho, mas não se preocupe porque eu sempre estarei aqui para ajudar!",
	...
	"Além dessas grandes dificuldades, também existem problemas no cenário econômico e na disponibilidade de recursos. É necessário tratar desses problemas antes de qualquer coisa!",
	"Para facilitar, vamos dividir o trabalho em três partes: ORÇAMENTO PÚBLICO, QUESTÃO AMBIENTAL e CONSTRUÇÃO DA OBRA PÚBLICA",
	"Os indicadores de aprovação da população - em vermelho - e do Congresso - em amarelo - estão localizados no canto superior direito da tela. Equilibre eles para finalizar melhor seu mandato.",
...
func _ready():
	...
	if Global.personagem == 1:
		$Presidente3.texture = load("res://imag/CardsNovos/President_1/01.png")
