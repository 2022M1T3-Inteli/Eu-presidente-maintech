...
func _process(delta):
	...
	if($Dinheiro.position.y >= 280): # Ao bater em baixo da tela
		indoE = false
		indoD = true

	if($Dinheiro.position.x <= -460): # Ao bater na esquerda da tela
		indoC = true
		indoB = false
		
	if($Dinheiro.position.x >= 480): # Ao bater na direita da tela
		indoC = false
		indoB = true
		
	if ($Dinheiro.position.y <= -280): # Ao bater em cima da tela
		indoD = false
		indoE = true
