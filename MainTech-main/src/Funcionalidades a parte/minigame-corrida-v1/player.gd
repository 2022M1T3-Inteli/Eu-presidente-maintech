...
func movimento_player(): # função de movimento do player
	if Input.is_action_just_pressed("ui_direita"): # o que acontece se o player apertar o botão de ir para a direita
		if $".".position.x == 512:
	...
	if Input.is_action_just_pressed("ui_esquerda"): # o que acontece se o player apertar o botão de ir para a direita
		if $".".position.x == 512:
